#!/usr/bin/env tsx
/**
 * Fund a wallet with mock USDC for testing on localnet.
 * Usage: npx tsx fund-wallet-localnet.ts <wallet-address> [amount-usdc]
 * 
...
async function main() {
  ...
  if (!walletArg) {
    console.error('Usage: npx tsx fund-wallet-localnet.ts <wallet-address> [amount-usdc]');
    process.exit(1);
  }
  ...
  if (!usdcMintStr) {
    console.error('❌ No usdc-mint found. Run bootstrap-localnet.ts first.');
    process.exit(1);
  }
